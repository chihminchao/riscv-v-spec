...
Yunsup Lee, Guy Lemieux, Filip Moc, Rich Newell, Albert Ou, David Patterson,
...
Attempts to access the `fcsr`, `vxsat`, or `vxrm` CSRs raise an illegal-instruction
exception when _both_ `VS` and the floating-point context status `FS` are set to Off.

If `VS` is set to Initial or Clean, executing any instruction that changes
`fcsr`, `vxsat`, or `vxrm` will change `VS` to Dirty.  If `FS` is set to Initial or Clean,
executing any instruction that changes `fcsr`, `vxsat`, or `vxrm` will change `FS` to Dirty.
(If either `VS` or `FS` is set to Off, it will remain Off.)

NOTE: Context-switching code must save all of `fcsr` if either `FS` or `VS` is set to Dirty.
...
----
        Product  Rounded                Guard
 SEW     Width   Product   Accumulator   Bits
   8       16       12         16         4
  16       32       24         32         8
  32       64       48         64        16
----
...
----
                        ...
                        Integer     Integer       FP sum/dot   FP sum/dot   FP sum/dot
                        sum         dot           accumulator  accumulator  accumulator
SEW  EDIV  Sub-element  accumulator accumulator    FLEN=32      FLEN=64      FLEN=128
  8b   2      4b          8b            8b          -            -            -
  8b   4      2b          8b            8b          -            -            -
  8b   8      1b          8b            8b          -            -            -
